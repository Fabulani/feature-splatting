...
Usage:
    python plot_feature_clusters.py [--results-dir clustering_results] [--labels-file labels.txt] [--output output.html]
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to text file containing labels to plot (one per line). Lines starting with # are ignored as comments. If not provided, plots all available labels.",
    )
    ...
    if args.labels_file:
        try:
            ...
            with open(labels_file_path, "r", encoding="utf-8") as f:
                labels_to_plot = [line.strip() for line in f if line.strip() and not line.strip().startswith("#")]
            print(f"Loaded {len(labels_to_plot)} labels from {labels_file_path}")
    ...
    try:
        data = load_clustering_results(args.results_dir, labels_to_plot)
