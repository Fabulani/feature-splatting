...
def batch_extract_feature(image_paths: List[str], args):
    ...
    # For part-level CLIP
    transform = T.Compose([T.Resize((args.part_resolution, args.part_resolution)), T.ToTensor(), norm])

    # For object-level CLIP
    raw_transform = T.Compose([T.ToTensor(), norm])
    ...
    for i in trange(len(image_paths)):
        ...
        resized_clip_feat_map_bchw = torch.nn.functional.interpolate(
            whole_image_feature.unsqueeze(0).float(), size=(object_H, object_W), mode="bilinear", align_corners=False
        )

        mask_tensor_bchw = sam_mask.unsqueeze(1)

        resized_mask_tensor_bchw = torch.nn.functional.interpolate(
            mask_tensor_bchw.float(), size=(object_H, object_W), mode="nearest"
        ).bool()
        ...
        aggregated_feat_map = F.interpolate(
            aggregated_feat_map[None], (final_H, final_W), mode="bilinear", align_corners=False
        )[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Intermediate (for MAP) SAM-enhanced Object-level feature resolution",
    )
    parser.add_argument(
        "--part_feat_res",
        type=int,
        default=300,
        help="Intermediate (for MAP) SAM-enhanced Part-level feature resolution",
    )
