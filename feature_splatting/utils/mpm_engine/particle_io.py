...
class ParticleIO:
    ...
    def write_particles(solver, fn, slice_size=1000000):
        ...
        for d in range(solver.dim):
            ...
            np_x = (
                (np_x - ranges[0, d, 0]) * (1 / (ranges[0, d, 1] - ranges[0, d, 0])) * (2**ParticleIO.x_bits - 1)
                + 0.499
            ).astype(np.uint32)
            np_v = (
                (np_v - ranges[1, d, 0]) * (1 / (ranges[1, d, 1] - ranges[1, d, 0])) * (2**ParticleIO.v_bits - 1)
                + 0.499
            ).astype(np.uint32)
    ...
    def read_particles(fn, dim):
        ...
        del data
        gc.collect()
        x = (x_and_v >> ParticleIO.v_bits).astype(np.float32) / (2**ParticleIO.x_bits - 1)
        for c in range(dim):
            x[:, c] = x[:, c] * (ranges[0, c, 1] - ranges[0, c, 0]) + ranges[0, c, 0]
        v = (x_and_v & (2**ParticleIO.v_bits - 1)).astype(np.float32) / (2**ParticleIO.v_bits - 1)
        for c in range(dim):
    ...
    def convert_particle_to_ply(fns):
        for fn in fns:
            ...
            color = (color[:, 2].astype(np.uint32) << 16) + (color[:, 1].astype(np.uint32) << 8) + color[:, 0]
            color = color[:, None]
