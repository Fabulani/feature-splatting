...
class gaussian_editor:
    ...
    def prepare_editing_dict(self, translation_vec, yaw_deg, physics_sim_flag):
        ...
        if "ground_R" in self.meta_editing_dict and "ground_T" in self.meta_editing_dict:
            ...
            if yaw_deg:
                ...
                rot_axis = self.meta_editing_dict["up_gravity_vec_np"] / np.linalg.norm(
                    self.meta_editing_dict["up_gravity_vec_np"]
                )
    ...
    def initialize_mpm_engine(self, youngs_modulus_scale=1, poisson_ratio=0.2):
        ...
        mpm = MPMSolver(
            res=(32, 32, 32), size=1, max_num_particles=2**21, E_scale=youngs_modulus_scale, poisson_ratio=poisson_ratio
        )

        self.meta_editing_dict["physics_sim"] = {"mpm": mpm, "gui": gui, "initialized": False}

    @torch.no_grad()
    def initialize_mpm_w_particles(
        self, init_particles_positions, infilling_downsample_ratio=0.2, ground_level=0.05, gravity=4
    ):
        ...
        support_particles = np.random.permutation(support_particles)[
            : int(len(support_particles) * infilling_downsample_ratio)
        ]
        ...
        shift_constant = np.array(
            [-particles[:, 0].mean() + 0.5, -particles[:, 1].min(), -particles[:, 2].mean() + 0.5]
        )

        particles += shift_constant

        self.meta_editing_dict["physics_sim"]["mpm"].add_particles(
            particles=particles, material=MPMSolver.material_sand, color=0xFFFF00
        )

        self.meta_editing_dict["physics_sim"]["mpm"].add_surface_collider(
            point=(0.0, ground_level, 0.0),
            normal=(0, 1, 0),
    ...
    def pre_rendering_process(self, means, opacities, scales, quats, editing_dict, view_main_obj_only=False, **kwargs):
        ...
        if "xyz_min" in self.meta_editing_dict:
            ...
            bbox_particle_idx = self.filter_particles_ground_bbox(means, kwargs["min_offset"], kwargs["max_offset"])
